...
func NewApp(kubeconfigPath string, debug bool) *App {
	...
	app := &App{
		...
		model: &models.AppModel{
			...
			StatusContainer:     components.NewStatusContainer(140, 15, "📊 Status e Informações"),
...
func (a *App) applyTerminalSizeLimit(width, height int) (int, int) {
	// Definir resolução mínima para garantir que todos os elementos sejam visíveis
	minWidth := layout.MinTerminalWidth   // Resolução mínima de largura - todos os painéis visíveis
	minHeight := layout.MinTerminalHeight // Resolução mínima de altura - layout completo visível

	a.debugLog("🔧 Checking limits: input=%dx%d, min=%dx%d", width, height, minWidth, minHeight)

	// Se o terminal for menor que o mínimo, ajustar para evitar elementos escondidos
	if width < minWidth {
		a.debugLog("🔧 Width too small: %d -> %d", width, minWidth)
		width = minWidth
	}
	if height < minHeight {
		a.debugLog("🔧 Height too small: %d -> %d", height, minHeight)
		height = minHeight
	}

	a.debugLog("🔧 Final size: %dx%d", width, height)
	return width, height
}
...
func (a *App) View() string {
	...
	// Garantir que sempre usamos as dimensões mínimas
	if a.width < layout.MinTerminalWidth || a.height < layout.MinTerminalHeight {
		a.debugLog("⚠️  View() detectou dimensões abaixo do mínimo: %dx%d, forçando %dx%d",
...
func (a *App) handleKeyPress(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "f7":
		// F7 tem dois comportamentos dependendo do estado:
		// - StateClusterSelection: Adicionar novo cluster
		// - Outros estados: Gerenciar recursos do cluster
		if a.model.State == models.StateClusterSelection {
			// Adicionar novo cluster
			a.model.State = models.StateAddingCluster
			a.model.AddingCluster = true
			// Inicializar campos do formulário
			a.model.AddClusterFormFields = map[string]string{
				"clusterName":    "",
				"resourceGroup":  "",
				"subscription":   "",
			}
			a.model.AddClusterFieldOrder = []string{"clusterName", "resourceGroup", "subscription"}
			a.model.AddClusterActiveField = "clusterName"
	...
	case "f9":
		...
		a.debugLog("🔧 F9 pressionado - carregando CronJobs do cluster %s", a.model.SelectedCluster.Name)
...
func (a *App) saveSession(sessionParam *models.Session) tea.Cmd {
	return func() tea.Msg {
		...
		// Adicionar mudanças dos HPAs selecionados
		for _, hpa := range a.model.SelectedHPAs {
			if hpa.Modified {
		...
		// Adicionar mudanças dos node pools selecionados
		for _, pool := range a.model.SelectedNodePools {
			if pool.Modified {
...
func (a *App) renderMixedSession() string {
	return "🔄 Sessão Mista (HPAs + Node Pools) - Em Implementação\n\n" +
...
func (a *App) loadCronJobs() tea.Cmd {
	return func() tea.Msg {
		...
		clusterName := a.model.SelectedCluster.Name
		a.debugLog("🔄 Carregando CronJobs do cluster: %s", clusterName)
