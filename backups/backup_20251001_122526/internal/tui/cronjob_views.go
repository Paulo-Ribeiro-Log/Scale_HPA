...
func (a *App) renderCronJobSelection() string {
	if a.model.Loading {
		...
		return renderPanelWithTitle(content, fmt.Sprintf("CronJobs do Cluster: %s", a.model.SelectedCluster.Name), 70, 18, primaryColor)
	}
	...
	return sessionInfo + cronJobPanel + statusSpacing + statusPanel
}
...
func (a *App) renderCronJobEditing() string {
	if a.model.EditingCronJob == nil {
		return "⚠️ Nenhum CronJob selecionado para edição"
	}
	...
	editingPanel := renderPanelWithTitle(content.String(), fmt.Sprintf("Editando CronJob: %s", cronJob.Name), 70, 18, primaryColor)

	// Painel de status
	statusPanel := a.renderStatusInfoPanel()
	statusSpacing := a.calculateCronJobEditingPanelSpacing()

	return sessionInfo + editingPanel + statusSpacing + statusPanel
}
