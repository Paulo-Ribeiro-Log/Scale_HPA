...
const Index = ({ onLogout }: IndexProps) => {
  ...
  const [hpaRefreshKey, setHpaRefreshKey] = useState(0); // 🔧 Forçar re-render da lista HPAs
  ...
  const { hpas, loading: hpasLoading, refetch: refetchHPAs, postApplyRefetch: postApplyRefetchHPAs, forceRefetchWithParams } = useHPAs(selectedCluster, selectedNamespace);
  ...
  // 🔧 FIX: Função para forçar refresh visual da tela de HPAs
  const forceHPARefresh = () => {
    console.log('[Index] Forçando refresh visual da tela de HPAs');
    setHpaRefreshKey(prev => prev + 1);
    setSelectedHPA(null); // Reset seleção para forçar atualização
  };

  // 🔧 FIX: Função para forçar reload do contexto sem verificar se cluster é igual
          ...
          <SplitView
            leftPanel={{
              ...
              content: hpasLoading ? (
                ...
                <div className="space-y-2" key={hpaRefreshKey}>
                  {hpas.map((hpa) => (
                    <HPAListItem
                      key={`${hpa.cluster}-${hpa.namespace}-${hpa.name}-${hpaRefreshKey}`}
                      name={hpa.name}
            ...
            rightPanel={{
              ...
              content: (
                <HPAEditor
                  ...
                  onApplied={() => {
                    // 🔧 AGUARDAR toast de status + 2s para recarregar contexto
                    setTimeout(() => {
                      console.log(`[Index] HPAEditor - Após toast de status - forçando reload do contexto`);
    ...
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <ApplyAllModal
        ...
        onApplied={() => {
          // 🔧 AGUARDAR toast de status + 2s para recarregar contexto
          setTimeout(() => {
            console.log(`[Index] Após toast de status - forçando reload do contexto`);
            forceReloadClusterContext();
          }, 4000);
        }}
        onClear={() => {
